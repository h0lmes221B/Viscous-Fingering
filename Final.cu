...
__global__ void definitions2(real* Ky, double interm, int* CELYG)
{
	int idy = blockIdx.x * blockDim.x + threadIdx.x;
	Ky[idy] = (((idy+CELYG[0]/2-1)%CELYG[0])-CELYG[0]/2+1)*interm;	
}
...
int main(int argc,char **argv)
{
    ...
    printf ( "Starl time and date: %s", asctime (timeinfo) );
	int seed;
...
void definitions(real Kx[],real Ky[],real K2[],real K2T[],real Dtk2[],real Dt2k2[])
{
	...
	interm = M_2PI/Ly;
	definitions2<<<1,128>>>(Ky,interm,CELYG);
