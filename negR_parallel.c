...
{
	//Printing the current local time of the system
	time_t rawtime1;
	struct tm * timeinfo1;

	time ( &rawtime1 );
	timeinfo1 = localtime ( &rawtime1 );
	printf ( "Start time and date: %s", asctime (timeinfo1) );
...
/*	seed = 1000;
	...
	printf ( "End time and date: %s", asctime (timeinfo2) );
	return 0;
}
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
/*void detecteur()
...
void premiere_fois()
{
	...
	for (id = 1; id < CELY*(CELX/2+1); ++id){
		PP[id].re = R*(NN[id].re+ccy[id].re)/k2t[id];
		PP[id].im = R*(NN[id].im+ccy[id].im)/k2t[id];
	}
       	PP[0].re = 0.;
	PP[0].im = 0.;
	free(ccy);
