...
__global__ void loop1int(real *J, real *N,real *Psix, real *Psiy, real *cx,real *cy)
{
...
double arand();
void definitions(real *a,real *aa,real *as,real *ds,real *vc,real *q);
void condition_initiale(int choix);
void dK_x(complexy *aa,complexy *aaa);
void dK_y(complexy *aa,complexy *aaa);
void lap();
void integration();
void read_parameters(char *file);
void premiere_fois();
void tfi(complexy *aa, real *sa);
...
void premiere_fois()
{
	...
	cudaALLOC(JJoldG,complexy,CELY*(CELX/2+1),premiere_fois);
...
void integration()
{
	int id,idy;
	real *J,*N,*JG,*NG;
	real *Psix,*PsixG,*Psiy,*PsiyG;
	real *cx,*cy,*cxG, *cyG, *Jnew,*cnew,*cnewG, *JnewG;
	...
	cudaALLOC(PsixG,real,CELX*CELY,integration);
	cudaALLOC(PsiyG,real,CELX*CELY,integration);
	cudaALLOC(cxG,real,CELX*CELY,integration);
	cudaALLOC(cyG,real,CELX*CELY,integration);
	...
	cudaMemcpy(PsixG, Psix,CELY*CELX*sizeof(real), cudaMemcpyHostToDevice);
	cudaMemcpy(PsiyG, Psiy,CELY*CELX*sizeof(real), cudaMemcpyHostToDevice);
	cudaMemcpy(cxG, cx,CELY*CELX*sizeof(real), cudaMemcpyHostToDevice);
	cudaMemcpy(cyG, cy,CELY*CELX*sizeof(real), cudaMemcpyHostToDevice);	

	loop1int<<<1024,1024>>>(JG,NG,PsixG,PsiyG,cxG,cyG);

	cudaMemcpy(J, JG, CELY*CELX*sizeof(real), cudaMemcpyDeviceToHost);
	cudaMemcpy(N, NG, CELY*CELX*sizeof(real), cudaMemcpyDeviceToHost);
	cudaFree(PsixG);
	cudaFree(PsiyG);
	cudaFree(cxG);
	cudaFree(cyG);
